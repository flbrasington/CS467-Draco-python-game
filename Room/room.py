...
class Room:
    def __init__(self, height, width, screenWidth, screenHeight, roomType):
        ...
        # roomWidth is the number of columns of cells
    ...
    def draw_screen(self, screen):
        ...
        for row in range(self.roomHeight):
            for col in range(self.roomWidth):
                pygame.draw.rect(screen, self.cells[row][col],
                                 [col * self.cellWidth, row * self.cellHeight, self.cellWidth - 10,
                                  self.cellHeight - 10], 0)
